...
def log(msg):
    try: bot.send_message(ADMIN_ID, f"🔧 {msg}")
    except: pass
...
def get_unit(symbol):
    if symbol.endswith("USDT"): return "پوینت"
    return "پیپ"
...
def get_forex_candles(symbol, interval):
    ...
    try:
        ...
        if "values" not in data:
            log(f"⚠️ {symbol}: {data.get('message', 'خطا')}")
            return None
...
def generate_analysis(signal, entry):
    if signal == "BUY":
        return (f"قیمت در ناحیه اشباع فروش قرار دارد.\n"
                f"احتمال بازگشت صعودی وجود دارد.\n"
                f"در صورت تثبیت بالای {entry:.5f}، ورود معتبر است.")
    else:
...
def check_signal(symbol, df, interval, market):
    ...
    if signal:
        ...
        signal_data = {
            "time": current_time, "symbol": symbol, "market": market, "type": signal,
...
def process_telegram_updates():
    ...
    if os.path.exists(offset_file):
        with open(offset_file, "r") as f: offset = int(f.read().strip() or 0)
        ...
        for u in updates:
            ...
            if "message" in u:
                ...
                if text == "/start":
                    ...
                    if chat_id == ADMIN_ID:
                        ...
                        try: bot.send_message(chat_id, f"✅ قبلاً تایید شده‌اید.\n\n📎 لینک کانال:\n{CHANNEL_INVITE_LINK}")
                        except: pass
                    elif any(p["chat_id"] == chat_id for p in pending["pending"]):
                        try: bot.send_message(chat_id, "⏳ در انتظار تایید ادمین.")
                        except: pass
                    else:
            ...
            if "callback_query" in u:
                ...
                if data_cb.startswith("approve_"):
                    ...
                    try: bot.edit_message_text(f"✅ کاربر {target_id} تایید شد.", chat_id=ADMIN_ID, message_id=cb["message"]["message_id"])
                    except: pass
                    try: bot.send_message(target_id, f"🎉 تایید شدید!\n\n📎 لینک کانال:\n{CHANNEL_INVITE_LINK}")
                    except: pass
                    ...
                    try: bot.edit_message_text(f"❌ کاربر {target_id} رد شد.", chat_id=ADMIN_ID, message_id=cb["message"]["message_id"])
                    except: pass
...
if __name__ == "__main__":
    ...
    for symbol in FOREX_SYMBOLS:
        for interval in FOREX_INTERVALS:
            try:
                ...
                check_signal(symbol, df, interval, "فارکس")
                count += 1
    ...
    if now_iran.weekday() == 5 and now_iran.hour == 10 and now_iran.minute < 25:
        try: weekly_report()
        except: pass

    if now_iran.weekday() == 4 and now_iran.hour == 23 and now_iran.minute < 25:
        try: weekend_message()
        except: pass
